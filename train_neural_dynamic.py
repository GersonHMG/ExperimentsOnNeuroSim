...
from models.neural_dynamic import GrSimDynamics
...
def main():
    ...
    run_name  = f"runs/neural_hybrid_{timestamp}"
    ...
    train_losses, val_losses = trainer.train(
        epochs=epochs,
        save_path="grsim_neural_dynamics_network.pth",
    )
    ...
    fig = evaluator.plot_trajectory(
        true_states,
        predicted_states,
        title=f"Neural-Hybrid Rollout (MEDE: {mede:.3f} m, MACE: {mace:.3f} rad)",
    )
