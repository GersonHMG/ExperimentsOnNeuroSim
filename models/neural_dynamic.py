...
class GrSimDynamics(DynamicsBase):
    def __init__(self):
        ...
        # ==========================================
        # 5. Residual Dynamics Neural Network (NEW)
        # ==========================================
        # Input: [vx, vy, omega, vx_cmd, vy_cmd, omega_cmd] (6 features)
        # Output: [F_x_res, F_y_res, Torque_w_res] (3 features)
        self.residual_net = nn.Sequential(
            nn.Linear(6, 32),
            nn.Tanh(), # Tanh is better bounded for predicting physical forces
            nn.Linear(32, 32),
            nn.Tanh(),
            nn.Linear(32, 3) 
        )
        
        # Initialize the final layer to near-zero. This ensures the model starts out 
        # acting purely like your analytical physics model before training begins.
        nn.init.uniform_(self.residual_net[-1].weight, -1e-5, 1e-5)
        nn.init.uniform_(self.residual_net[-1].bias, -1e-5, 1e-5)
    ...
    def _compute_delta_internal(self, state: State, command: Command) -> Tuple[State, torch.Tensor]:
        """
        Core dynamics computation.
        Returns both the state delta and the residual forces predicted by the neural network.
        """
        dt = self.dt
        ...
        # Calculate ideal analytical body forces
        F_req_x = self.body_mass * a_req_x
        F_req_y = self.body_mass * a_req_y
        T_req_w = self.body_inertia * a_req_w
        
        # Stack into Base Analytical Body Force vector: Shape (Batch, 3)
        F_body_analytical = torch.stack([F_req_x, F_req_y, T_req_w], dim=-1)

        # ------------------------------------------
        # NEW: Predict unmodeled residual forces
        # ------------------------------------------
        # Stack inputs for the neural network. (Batch, 6)
        nn_inputs = torch.stack([
            vx_curr_local, vy_curr_local, state.omega,
            vx_cmd_limited, vy_cmd_limited, omega_cmd_limited
        ], dim=-1)
        
        # Output is the "missing" forces (Batch, 3)
        F_body_residual = self.residual_net(nn_inputs)
        
        # Combine ideal physics with neural network corrections
        ...
        # Use pseudo-inverse of H^T to distribute required body force across wheels optimally
        H_T = H.T  # Shape: (3, 4)
        H_T_pinv = torch.linalg.pinv(H_T) # Shape: (4, 3)
        
        # Batched matrix multiplication: (Batch, 3) @ (3, 4) -> (Batch, 4) wheel forces
        ...
        # Clamp individual wheel forces (Limits the NN from cheating physics constraints)
        ...
        # Calculate the true resulting accelerations
    ...
    def compute_delta(self, state: State, command: Command) -> State:
        """Required by DynamicsBase. Discards residual forces for standard forward pass."""
        delta, _ = self._compute_delta_internal(state, command)
        return delta

    def forward_with_residual(
        self, state_tensor: torch.Tensor, cmd_tensor: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Extended forward pass used during hybrid training.
        Returns:
            next_state_tensor: Shape (Batch, 6)
            residual_forces:   Shape (Batch, 3) — F_body_residual from the neural network
        """
